...
- **🏹 눌림목·반등목 토픽(2026-08-23 신설, 2026-09-11 미사용)**: BTC=1619/XAU=1620. **2026-09-11 방 통합으로 `emit_pullback`이 맥점 토픽으로 리다이렉트**돼 더 이상 여기로 안 감(토픽은 잔존).
- **⭐ 방 통합(2026-09-11)**: 막돌파·맥점·눌림목 전부 **맥점 토픽 하나로**(BTC 3분=558/5분=574/10분=911, 유일 잔존). `emit_breakout`·`emit_pullback`→`tg_send`. **XAU 전면 중단**(xau-3m/5m/10m stop+disable). 헬스체크 BTC 3봇. **카드 첫 줄에 종류 표시**(`sig_type` 헤더: 눌림목>맥점완성>막돌파). **안 쓰는 토픽 11개 삭제**(맥점 XAU 620/621/912+눌림목 1619/1620; 막돌파 BTC 14/132/403+XAU 205/206/404). 막돌파 그룹(-1004425656249)은 빈 그룹으로 잔존.
...
- **발송처**: 기존 그룹 2개 재사용, XAU 전용 토픽 신설(봇 토큰·chat_id 재사용).
  - 맥점신호 그룹(-1003964313330) XAU 토픽: 3분봉=620 / 5분봉=621 / **10분봉=912** / 30분봉=622 / (1시간봉 토픽 623 삭제)
  - 막돌파신호 그룹(-1004425656249) XAU 토픽: 3분봉=205 / 5분봉=206 / **10분봉=404** / 30분봉=207 / (1시간봉 토픽 208 삭제)
...
- **재시작**: `sudo systemctl restart maekjeom-bot-xau-3m maekjeom-bot-xau-5m maekjeom-bot-xau-10m maekjeom-bot-xau-30m`
- **상태**: `systemctl is-active maekjeom-bot-xau-{3m,5m,10m,30m}` / `ps aux|grep ws_watch` → BTC4+XAU4 = **8개**
