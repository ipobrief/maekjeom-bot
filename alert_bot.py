...
def fmt_boss(e, long_, htf_labels):
    """큰형님 우선의 법칙(책 부록): 상위 3개 TF 각각 MACD 0선·스토 50 위치가 신호 방향과 정렬됐는지.
    표시 전용(발송 억제 아님). 롱=MACD 0선 위 & 스토 50 위 / 숏=거울(아래). (2026-08-21 크로스→0선/50 위치 복귀)"""
...
def boss_aligned(e, long_, need=4):
    """큰형님(상위3TF MACD 0선·스토 50 위치) 6개 항목 중 need개 이상 신호방향 정렬이면 True(추세정렬).
    미만이면 역추세 경고. 역추세 판정을 큰형님 기준으로 통일(2026-08-18). 기준: 0선/50 위치(2026-08-21 크로스에서 복귀)."""
    m0 = e.get("boss_m0") or [False, False, False]
    st = e.get("boss_st") or [False, False, False]
