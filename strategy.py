...
def pullback_structure(df, ma, L=3, R=3, lookback=120, only_last=90, ma_band=0.004, touch=5):
    ...
    for i in range(start, n):
        ...
        # ① 롱 HL / 숏 LH  (종가 기준 — 윗꼬리/아랫꼬리 무시, 2026-08-29 사용자 확정)
        if len(lows) >= 2:
...
def build_signals(df15, df1h, df4h, df1d, cfg):
    ...
    # ── 큰형님 우선의 법칙(책 부록): 상위 3개 TF(df1h·df4h·df1d) 각각 MACD 0선·스토 50 확인 ──
    #   기준 변천: GC/DC → 0선/50(2026-08-18) → GC/DC(2026-08-20) → 0선/50 복귀(2026-08-21).
    #   사유(2026-08-21): DC가 나도 MACD 0선 위·스토 50 위면 추세 유지(오늘 BTC 상승 지속) →
    #     '위치'가 추세정렬을 더 잘 반영. 큰형님은 추세방향 판정이라 크로스보다 0선/50선이 적합.
