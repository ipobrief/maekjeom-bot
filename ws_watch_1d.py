...
def bo_ready():
    return bool(os.environ.get("TELEGRAM_CHAT_ID_BO") and os.environ.get("TELEGRAM_BO_THREAD_1D"))
...
def emit_breakout(text):
    ...
    return ab.tg_send_room(text, os.environ.get("TELEGRAM_TOKEN_1D"),
                           ...
                           os.environ.get("TELEGRAM_BO_THREAD_1D"))
        ...
        if d and breakout and getattr(handle_tick, "send_confirm", True):
            if aligned:                              # 맥점방(완성형)
                if st.sent_key != (d, when):
                    ...
                    print(f"[ws-1d] {kst(when):%m-%d} 마감: 맥점방 중복")
            elif bo_ready() and d != st.bo_last_dir and st.sent_bo != (d, when):   # 막돌파방
                emit_breakout(fmt_signal(e, when, provisional=False))
                st.sent_bo = (d, when)
                st.bo_last_dir = d
            else:
                print(f"[ws-1d] {kst(when):%m-%d} 마감: 막돌파방 억제")
        else:
            print(f"[ws-1d] {kst(when):%m-%d} 마감: {(d and '막돌파아님') or '신호없음'}")
        divergence.check(st.df1d, SYMBOL, TF,
                         os.environ.get("TELEGRAM_TOKEN_1D"),
                         os.environ.get("TELEGRAM_CHAT_ID_1D"),
                         os.environ.get("TELEGRAM_DIV_THREAD_1D") or os.environ.get("TELEGRAM_THREAD_ID_1D"),
                         st.sent_div)
        st.alerted_bar = None
        st.alerted_dirs = set()
        return
