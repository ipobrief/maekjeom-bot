...
def fmt_stoch_regime(zone, k, when):
    ...
    if zone == "hot_clear":
        return (
            f"🥵➡️😌 <b>30분봉 스토 과열 해소</b> (%K {k:.0f} < {STOCH_HOT})\n"
            f"━━━━━━━━━━━━━\n"
    ...
    return (  # cold_clear
        f"🥶➡️😌 <b>30분봉 스토 침체 해소</b> (%K {k:.0f} > {STOCH_COLD})\n"
        f"━━━━━━━━━━━━━\n"
...
def handle_tick(st, k):
    ...
    if is_closed:
        ...
        # ── 30분봉 스토 과열/침체 알림: 진입 시 1회 + 해소(존 이탈) 시 1회 ──
        ...
        if getattr(handle_tick, "send_confirm", True) and k_now is not None:
            ...
            if prev_zone == "hot" and zone != "hot":
                emit(fmt_stoch_regime("hot_clear", k_now, when))
            elif prev_zone == "cold" and zone != "cold":
                emit(fmt_stoch_regime("cold_clear", k_now, when))
        st.alerted_bar = None
        st.alerted_dirs = set()
        return
