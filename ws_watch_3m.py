        ...
        if d and breakout and must_ok and getattr(handle_tick, "send_confirm", True):
            if pb_ready() and ab.pullback_note(e, d == "LONG"):   # 눌림목/반등목 → 눌림목방에만
                if st.sent_pb == (d, when):
                    print(f"[ws-3m] {kst(when):%m-%d %H:%M} 마감: 눌림목방 중복")
                elif not st.pb_ok(d, when):
                    print(f"[ws-3m] {kst(when):%m-%d %H:%M} 마감: 눌림목방 30분내 같은방향 억제")
                else:
                ...
                if st.sent_key == (d, when):
                    print(f"[ws-3m] {kst(when):%m-%d %H:%M} 마감: 맥점방 중복")
                elif not st.maek_ok(d, when):
                    print(f"[ws-3m] {kst(when):%m-%d %H:%M} 마감: 맥점방 30분내 같은방향 억제")
                else:
            ...
            print(f"[ws-3m] {kst(when):%m-%d %H:%M} 마감: {reason} | "
