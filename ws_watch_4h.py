# -*- coding: utf-8 -*-
"""맥점 웹소켓 실시간 감시 — 4시간봉(4h) 전용.

ws_watch.py(1h)·ws_watch_1m.py(15m)와 동일 구조. 15m/1h/1d 사이 중간 스윙 맥점 포착용.
알림 채널: 4시간봉 (TELEGRAM_TOKEN_4H / TELEGRAM_CHAT_ID_4H, 없으면 1D 채널 폴백).
4h 마감 = UTC 00/04/08/12/16/20 = KST 09/13/17/21/01/05.

실행:
  python ws_watch_4h.py                 # 예비+확정 모두 발송
  python ws_watch_4h.py --no-confirm    # 예비만 발송
"""
import os
import sys
import json
...
HTF_REFRESH_SEC = 1800       # 상위TF 갱신 주기(30분)
RECOMPUTE_MIN_SEC = 300      # 잠정 재계산 최소 간격(5분)
PROV_MIN_MINS_LEFT = 30      # 마감 30분 미만이면 잠정 억제
...
def tg_send(text):
    token = os.environ.get("TELEGRAM_TOKEN_4H") or os.environ.get("TELEGRAM_TOKEN_1D")
    chat = os.environ.get("TELEGRAM_CHAT_ID_4H") or os.environ.get("TELEGRAM_CHAT_ID_1D")
    if not token or not chat:
        print("⚠️ 4H봇 텔레그램 미설정: TELEGRAM_TOKEN_4H/1D 없음 (콘솔만 출력).")
        return False
    url = f"https://api.telegram.org/bot{token}/sendMessage"
    payload = {"chat_id": chat, "text": ab.tg_html(text), "parse_mode": "HTML"}
    thread = os.environ.get("TELEGRAM_THREAD_ID_4H")   # 그룹 토픽(4시간봉)
    if thread:
    ...
    try:
        ...
        j = r.json()
        if j.get("ok"):
            print("✅ 텔레그램(4h) 전송 성공(평문)")
            return True
        print(f"❌ 텔레그램(4h) 전송 실패: {j.get('description')}")
        return False
    except Exception as e:
        print("❌ 텔레그램(4h) 전송 오류:", e)
        return False
...
class LiveState:
    ...
    def load_base(self):
        self.df4h = data.get_history(SYMBOL, TF, bars=600)
        self._load_htf()

    def _load_htf(self):
        self.dfA = data.get_history(SYMBOL, HTF[0], bars=300)
    ...
    def upsert_bar(self, k):
        ...
        row = {"open": float(k["o"]), "high": float(k["h"]),
               ...
               "volume": float(k["v"])}
        self.df4h.loc[t] = row
        self.df4h = self.df4h[~self.df4h.index.duplicated(keep="last")].sort_index().tail(600)
        return t

    def evaluate(self, idx):
        sig = strategy.build_signals(self.df4h, self.dfA, self.dfB, self.dfC, CFG)
...
def handle_tick(st, k):
    ...
    if aligned:
        gate = d not in st.alerted_dirs   # 맥점방
    else:
...
async def run(send_confirm=True):
    ...
    print(f"📡 맥점 웹소켓 감시 시작 — {SYMBOL} {TF} "
          ...
          + ("" if (os.environ.get("TELEGRAM_TOKEN_4H") or os.environ.get("TELEGRAM_TOKEN_1D")) else " (콘솔 모드)"))
    while True:
        try:
            ...
            try:
                st.df4h = data.get_history(SYMBOL, TF, bars=600)
            except Exception:
                pass
